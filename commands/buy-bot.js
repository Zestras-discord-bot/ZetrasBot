...
module.exports = {
    async run(discord, bot, message, args) {
        ...
        const paymentMethodCollector = ticket.createMessageCollector(
            (x) => true
        );
        ...
        paymentMethodCollector.on("collect", (message) => {
            ...
            if (
                paymentMethodContent === "bitcoin" ||
                paymentMethodContent === "2"
            ) {
                ticket.send(
                ...
                acceptedTermsCollector.on("collect", (message) => {
                    ...
                    if (
                        acceptedTermsContent === "no" ||
                        acceptedTermsContent === "2"
                    ) {
                        ...
                        whatLookingForCollector.on("collect", (message) => {
                            ...
                            if (
                                ...
                                whichServiceCollector.on(
                                    "collect",
                                    (message) => {
                                        collectorContent = message.content.toLowerCase();
                                        let selectedService =
                                            activeServicesList[
                                                parseInt(collectorContent) - 1
                                            ];
                                                ...
                                                (message) => {
                                                    ...
                                                    if (
                                                        ...
                                                        howManyCollector.on(
                                                            "collect",
                                                            (message) => {
                                                                const howManyContent = parseInt(
                                                                    message.content
                                                                );

                                                                if (
                                                                    howManyContent.toString() ===
                                                                    "NaN"
                                                                )
                                                                    return;
                                                                else if (
